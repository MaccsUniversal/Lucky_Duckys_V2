...
var SlotMachine = artifacts.require('Slot_Machine');
...
contract('SlotMachine', function(accounts) {
    ...
    beforeEach(async () => {
        instance = await SlotMachine.new();
    })

    xit("should deposit correct amount to contract for one player", async() => {
    ...
    xit("should deposit correct amount for multiple players", async() =>{
    ...
    xit("should withdraw players entire balance from the contract", async() => {
    ...
    xit("should withdraw the players balance but leave bets on the table", async() => {
        await instance.deposit({from: player2, value: deposit2});
        ...
        let withdrawalAmount = new BigNumber(fromWei(result.logs[0].args.amount));
    ...
    it("should reduce player.placedBet value and increase player.balance by desired amount", async() => {
        ...
        let uAmount = new BigNumber(fromWei(result.logs[0].args.amount));
        p1 = await instance.player(player1);
